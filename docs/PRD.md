...
### Slip storage behavior

payment slip ตอนนี้รองรับทั้งสองแบบ:

- ถ้ามี `BLOB_READ_WRITE_TOKEN` จะเก็บที่ Vercel Blob
- ถ้าไม่มี token จะ fallback ไปเก็บที่ `public/uploads/` ระหว่าง local development

สำหรับ Vercel preview/production ควรเปิดใช้ Blob store และควบคุมขนาดไฟล์ไว้ไม่เกิน `4.5MB`
